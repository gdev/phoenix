...
function moveDown() {
    ...
    var top = screen.frame().y;
    var bottom = (window.frame().y + window.frame().height)
    if (isApproximately(bottom, screen.frame().height, 5)) {
        window.setFrame({
            x: window.frame().x,
...
// This is used instead of a direct comparison of widths to accommodate apps like Terminal that don't
// allow precise window sizing!
...
function moveLateral(sectorsPerScreen, sectorMovement) {
    ...
    if (currentSector != null) {
        ...
        if (currentFrame.x < window.frame().x || currentFrame.x == window.frame().x && !isApproximately(currentFrame.width, window.frame().width, 5) == true && sectorMovement == -1) {
            sectorMovement = 0;
        };
